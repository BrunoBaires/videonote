...
// Manifiesto en logos/manifest.json: { groups:[...], items:[{id,group,name,url}] }
// Las imágenes se guardan como logos/<id>.<ext> (acceso público).
//
// GET                                  -> manifiesto { groups, items }
// POST { action:'upload', group, name, dataUrl }   -> sube un logo
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'upload') {
        ...
        if (buf.length > 4 * 1024 * 1024) { res.status(400).json({ error: 'Imagen muy pesada (máx 4 MB).' }); return; }
        const id = newId();
        ...
        m.items.push({ id, group, name, url: blob.url });
